...
int main(int argc, char *argv[])
{
    ...
    switch(metodo)
    {
        case 1:
        {
			...
			rewind(fitas[0]);
			i=0;
			while(fread(&bloco,sizeof(TipoBloco),1,fitas[0]))
			{
			...
			i=0;
			rewind(fitas[1]);
			while(fread(&bloco,sizeof(TipoBloco),1,fitas[1]))
			{
				printf("Matricula 2: %ld Nota: %.1f\n",bloco.v[i].mat,bloco.v[i].nota);
				i++;
			}
			i=0;
			rewind(fitas[2]);
			while(fread(&bloco,sizeof(TipoBloco),1,fitas[2]))
			{
				printf("Matricula 3: %ld Nota: %.1f\n",bloco.v[i].mat,bloco.v[i].nota);
				i++;
			}
			*/
			fclose(arq);
        ...
        case 4:
        {
            ...
            {
                printf("Cont: %d Matricula: %-8ld Nota: %.1f Estado: %s Cidade: %s Curso: %s\n",cont,reg.mat,reg.nota,reg.estado,reg.cidade,reg.curso);
