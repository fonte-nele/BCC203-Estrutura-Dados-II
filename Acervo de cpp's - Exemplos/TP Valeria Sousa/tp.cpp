...
void leituraclientes(ifstream& arqclientes, cliente vetor[], int *contador);
void leituradestinos(ifstream& arqdestinos, destino vetor2[], int *cont);
void leituraviagens(ifstream& arqviagens, viagem vetor3[], int *contar, destino vetor2[], int cont);

void escritaclientes(ofstream& arqclientes2, cliente vetor[], int contador);
void escritadestinos(ofstream& arqdestinos2, destino vetor2[], int cont);
void escritaviagens(ofstream& arqviagens2, viagem vetor3[], int contar);
...
int main()
{
	...
	arqdestinos.open("destinos.txt",ios_base::in);
	arqviagens.open("viagens.txt",ios_base::in);

	long int id;
	int opc, opcao, contador = 0, cont = 0, contar = 0, dia_inicial, dia_final, mes_inicial, mes_final; // n = tamanho do vetor, opcao = numero da escolha feita no menu
    ...
    leituraclientes(arqclientes, vetor, &contador);
	leituradestinos(arqdestinos, vetor2, &cont);
	leituraviagens(arqviagens, vetor3, &contar, vetor2, cont);
	...
	arqdestinos2.open("destinos.txt",ios_base::out);
	arqviagens2.open("viagens.txt",ios_base::out);
    
    escritaclientes(arqclientes2, vetor, contador);
...
void leituraclientes(ifstream& arqclientes, cliente vetor[], int *contador)
{
	...
	while(!arqclientes.eof())
	{
		...
		getline(arqclientes,line);
		vetor[(*contador)].telefone = line;

		cout << vetor[(*contador)].id << endl;
		cout << vetor[(*contador)].nome << endl;
		cout << vetor[(*contador)].endereco << endl;
		cout << vetor[(*contador)].email << endl;
		cout << vetor[(*contador)].telefone << endl;
		(*contador)++;
	}
}
...
void leituradestinos(ifstream& arqdestinos, destino vetor2[], int cont)
{
	char aux[20];
	string line;
	while(!arqdestinos.eof())
	{
		getline(arqdestinos,line);
		int tam = line.size();
        for(int j = 0; j < tam; j++)
		{
			aux[j] = line[j];
		}
		vetor2[cont].id = atol(aux);

		getline(arqdestinos,line);
		vetor2[cont].localidade = line;

		getline(arqdestinos,line);
		vetor2[cont].pais = line;
...
void leituraviagens(ifstream &arqviagens, viagem vetor3[], int *contar, destino vetor2[], int cont)
{
	...
	char aux[20];
	string line;
	while(!arqviagens.eof())
	{
		getline(arqviagens,line);
        int tam = line.size();
		for(int j = 0; j < tam; j++)
		{
			aux[j] = line[j];
		}
		vetor3[(*contar)].id = atol(aux);
        
        for(int i=0; i<cont; i++)
        {
            if(vetor3[(*contar)].id == vetor2[i].id)
            {
		...
		vetor[contar].idclientes = atol(aux);
...
void insercaov(cliente vetor[], destino vetor2[], viagem vetor3[], int *contar, int contador, int cont) // inserindo uma nova viagem 
{
    ...
    vetor3[(*contar)].id = id;
    // destino inicial sera sempre ouro preto
	...
	for(int i = 0; i < cont; i++)
	{
		if(id == vetor2[i].id) // Verifica se o destino pretendido existe dentro do vetor de destino
		{
            ...
            vetor3[(*contar)].quantd = vetor2[i].num+1; // quantd quantidade de deslocamentos em uma viagem
