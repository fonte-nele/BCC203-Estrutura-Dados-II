			...
			while (atualizaPonteiro == 1){
				...
				if (auxAtualiza.registro.chave < auxArvore.registro.chave){
					...
					if (auxArvore.esq == -1){ //verifica se tem filhos à esquerda
						...
						fseek (arqArvore, i*sizeof(TArvore), 0); //como fread avança uma posição, tem que dar fssek de novo, pra voltar pra posicao que tem que alterar
	...
	while (true) {
		...
		if (arvoreBusca.registro.chave == key){
			*reg = arvoreBusca.registro;
			...
			cout << "arvoreBusca.chave: " << arvoreBusca.registro.chave << endl;
			cout << "arvoreBusca.esq:   " << arvoreBusca.esq << endl;
			cout << "arvoreBusca.dir:   " << arvoreBusca.dir << endl;
